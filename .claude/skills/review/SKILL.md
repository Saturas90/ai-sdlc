...
2. Reviewer nach Risiko wählen: Standard → **`reviewer`** (sonnet); `[K]`/sicherheits-/datenkritisch → **`reviewer-kritisch`** (opus); Architekturplan → **`reviewer-architektur`** (opus, xhigh). Er liefert Findings: je `Kategorie | Fundstelle | Problem | Vorschlag`, plus `Blockierend: ja/nein`.
